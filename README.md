...
Bot 还会每 60 秒读取一次配置所涉及账号的 Sub2API 周窗口快照；相邻 7 日重置时间至少前移 1 小时才视为新窗口。Bot 先发送 3 分钟审批消息，管理员同意或超时后才备份并重置；管理员拒绝则跳过本次事件。
...
- 绑定账号的 7 日重置时间至少前移 1 小时后进入审批；3 分钟内同意则立即执行、拒绝则跳过、无人操作则自动执行，并向所有管理员发送逐 Key 成功或失败通知。
- 手动和自动重置前都会完整备份三个用量及三个窗口开始时间，每个 Key 保留最近 3 份；管理员可在 `/check` 菜单中选择备份回滚。
...
启用 Sub2API 管理接口后，Bot 默认每 60 秒通过固定只读函数检查一次这些绑定账号的 `codex_7d_reset_at`。首次看到账号时只保存基线，不会执行重置；相邻观测时间至少前移 1 小时才视为新窗口，秒级漂移只更新基线。检测到新窗口后向所有管理员发送审批按钮：第一个决定生效，同意立即执行，拒绝跳过，3 分钟无人操作则自动执行。同一 Key 去重，未出现在 `bindings` 中的 Key 不受影响；失败项保留到下次检查重试，每次成功或失败都会通知 `admins`。审批与检测状态保存在持久化卷中的 `auto_reset_state.json`，不会因为正常重启丢失。
...
普通用户直接查询自己的 Key；管理员会先看到 Key 查询按钮、“Key 总览”、“批量重置速率限制”和“回滚 Key 使用量”按钮。“Key 总览”按 Key 名称去重，每页显示 8 个 Key，展示最后使用时间、每周已用/限额/剩余和 12 格进度条；下方按 `account_id` 去重展示所有绑定账号的周使用百分比、周重置时间及剩余时长、账号消耗金额、满额金额预估及汇总，最后显示本次刷新时间。账号名称若为邮箱格式会自动脱敏。总览支持刷新、翻页和返回，不显示请求、Tokens、模型、5 小时或每日信息。
...
进入批量重置后，可以逐项勾选或取消 Key，也可以全选或清空；点击“重置所选”后还需最终确认。Bot 会先为每个 Key 备份 `usage_5h`、`usage_1d`、`usage_7d`、`window_5h_start`、`window_1d_start`、`window_7d_start`，并保存重置前的 `last_used_at` 与 `rate_limit_7d` 用于通知展示；备份成功才调用管理接口重置并复查。单个 Key 失败不会中断其余 Key，完成消息会分别汇总成功、需复查和失败数量，重置成功项会另行向管理员发送与 Key 总览相同格式的重置前数据。

每个 Key 只保留最近 3 份重置前备份。在“回滚 Key 使用量”中选择 Key、备份并再次确认后，Bot 会先通过官方管理接口定向清除该 Key 的数据库计数与 Redis 限速缓存，再立即从备份恢复上述六个字段并复查。回滚不修改 `quota_used`，也不删除历史用量记录。
...
自动重置与手动重置使用相同的 Sub2API 官方管理员接口，并且都要求备份成功后才执行。自动检测只读取 Sub2API 已保存的账号快照，不直接请求 OpenAI/Claude 官方接口。检查频率可通过 `.env` 中的 `SUB2API_TG_BOT_AUTO_RESET_CHECK_INTERVAL` 调整，允许范围为 60～3600 秒。
...
- 自动重置等待中：检查管理员私聊中的审批消息；通知成功后 3 分钟无人操作才会自动执行，审批状态会跨重启保留。
- 回滚列表为空：只有升级后实际执行过且备份成功的手动或自动重置才会产生备份；升级前的误重置不会自动补出旧数据。
