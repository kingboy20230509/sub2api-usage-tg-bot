...
Bot 每 60 秒分别读取各个已绑定上游账号保存的 7 日重置快照。第一次读取只建立基线；新时间在原计划时间到达后出现属于自然重置，Bot 只更新该账号监测状态，不重置任何 Key，也不发送消息。如果快照显示 7 日用量仍为 0、剩余时间接近完整窗口，并且绝对重置时间与快照更新时间一起向后移动，Bot 会将其视为尚未固定的滚动窗口，保持静默直到后续新快照显示绝对重置时间稳定或窗口已经开始消耗。只有原计划时间尚未到、且排除上述滚动窗口后，新时间仍向后推进至少 1 小时，才判定为 OpenAI 突然重置，并向唯一管理员发送以“通知：”开头的询问。管理员可以选择“对齐并重置此账号 Key”或“本次不重置”；3 分钟未操作会自动执行。确认、拒绝或超时后，该账号会等待新窗口固定，期间不会对同一次上游事件重复询问。多个账号先后或同时触发时，事件会保存在状态文件中并逐个询问，不会用一个账号的时间处理另一个账号的 Key。
